...
class Model {
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),$this->methods,true)) {
            ...
            return $this;
        }elseif(in_array(strtolower($method),array('count','sum'),true)){
            // 统计查询的实现
            $field =  isset($args[0])?$args[0]:'*';
            return $this->getField(strtoupper($method).'('.$field.') AS tp_'.$method) ? : 0;
        }elseif(in_array(strtolower($method),array('min','max','avg'),true)){
            // 统计查询的实现
            $field =  isset($args[0])?$args[0]:'*';
    ...
    public function select($options=array()) {
		...
		if(isset($options['trimQuote']) && $options['trimQuote']){ //清空json_encode时多余的引号,原因：整形的字段返回时也是按字符串返回了
			if(file_exists(CONF_PATH . 'db_structure.php'))
				$dbst = require(CONF_PATH . 'db_structure.php');
			else
				$dbst = array();
			$field_list = $dbst[$this->trueTableName]['field_list'];
			$fdst = array();
			foreach($field_list as $field => $info){
				if( in_array($info['FieldType'],array('auto_increment','enumerate','integer')) )
					array_push($fdst,$field);
			}
			foreach($resultSet as $key=>$row){
				foreach($fdst as $field){
					if(isset($row[$field])){
        ...
        if(isset($options['where']) && is_array($options['where']) && !empty($fields) && !isset($options['join'])) {
            ...
            foreach ($options['where'] as $key=>$val){
                ...
                if(in_array($key,$fields,true)){
					...
					//不要去掉不存在的where字段，不然写错了字段名称的话，可能会造成错误
					throw_exception(L('_FIELD_NAME_INVALID_') . ' ' .$key);
    ...
    protected function _parseType(&$data,$key) {
        if(empty($this->options['bind'][':'.$key])){
            ...
            if(false !== strpos($fieldType,'enum')){
				...
				if(!is_numeric($data[$key]))  //检测是不是数值类型，避免强制转换类型导致程序出错。例如 intval('1234,12') = 1234
					throw_exception(L('_FIELD_TYPE_INVALID_') . ' field:' .$key . ' fieldType:'.$fieldType . ' dataType:'.gettype($data[$key]) .' dataValue:'.$data[$key] );
				...
				if(gettype($data[$key]) != 'boolean')  //检测是不是数值类型，避免强制转换类型导致程序出错。例如 intval('1234,12') = 1234
					throw_exception(L('_FIELD_TYPE_INVALID_') . '  field:' .$key . ' fieldType:'.$fieldType . ' dataType:'.gettype($data[$key]) .' dataValue:'.$data[$key]);
    ...
    public function where($where,$parse=null){
		...
		if(is_numeric($where) ){ //补丁：增加对整形的支持（redis的键值对，可能键是整形，浮点数之类的其他类型就不允许做键）
            $map    =   array();
            $map['_string']   =  (string) $where;
            $where  =   $map;
        } 
	...
	/*
		张晓武添加的扩展方法
	*/
	public function createAdd($data='',$options=array(),$replace=false){
        if(! $this->create($data)){
			//json_fail('create fail' . $this->getModelName() );
            return false;
		}
		return $this->add('',$options,$replace);
	}
	public function createSave($cond='',$data='',$options=array()){
		if(!$this->create($data)){
			return false;
		}
		if($cond){
			return $this->where($cond)->save('',$options)!==false;
		}
		else{
			return $this->save('',$options)!==false;
		}
	}
	public function createAddSave($cond='',$data='',$options=array(),$replace=false){
		if($this->where($cond)->count() == 0)
			return $this->createAdd($data,$options,$replace);
		else
			return $this->createSave($cond,$data,$options);
	}
	public function createAddSaveEx($cond='',$add_data='',$save_data='',$options=array(),$replace=false){
		if($this->where($cond)->count() == 0)
			return $this->createAdd($add_data,$options,$replace);
		else{
			if(! $save_data )
				$save_data = $add_data;
			return $this->createSave($cond,$save_data,$options);
		}
	}
	public function getInfo($cond,$field='',$order=''){
		$db = $this->where($cond);
		if($order)
			$db=$db->order($order);
		if($field)
	...
	public function getList($cond,$limit=''){
		if($limit)
	...
	public function getPageList($cond,$order=''){
		$count = $this->getCount($cond);
		$pager = pager($count);
		$result = array();
		$result['page'] = $pager['page'];
		$result['count'] = $count;
		$result['list'] = $this->getList($cond,$pager['limit'],$order);
		return $result;
	}
	public function getCount($cond){
		return $this->where($cond)->count();
	}
	public function getConcat($field,$cond,$limit='',$order=''){
		if($limit){
			$db = $this->field($field)->where($cond)->limit($limit);
			if($order)
				$db = $db->order($order);
			$result = $db->select();
			$str = '';
			...
			$result = $this->field("group_concat($field) as gc_fields")->where($cond)->find();
			return $result['gc_fields'];
		}
	}	
}