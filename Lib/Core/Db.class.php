...
class Db {
        ...
        if ( !empty($db_config) && is_string($db_config)) {
			...
			if( 'pdo' == strtolower(C('DB_TYPE')) ){
                $db_config = array (
                    ...
                    'hostport'  =>  C('DB_PORT'),
                    'database'  =>  C('DB_NAME'),
                    'dsn'       =>  C('DB_NAME'),  //PDO类型的数据库，使用DB_NAME作为dsn配置,如果用DB_DSN会有冲突
                    'params'    =>  C('DB_PARAMS'),
                );
            }
			else{
				if( C('DB_DSN') ) { //优先使用DB_DSN的配置
    ...
    protected function needConnectDB($linkNum=0){
    ...
    protected function parseJoin($join) {
		...
		//preg_replace 有内存泄漏，如果不需要调用，直接跳过
		if($joinStr == '')
			return '';
    ...
    public function update($data,$options) {
		...
		if(!$where){
			//不允许条件为空，造成整表更新
			throw_exception(L('_SQL_WHERE_INVALID_'));
			return false;
		}
        $sql   = 'UPDATE '
