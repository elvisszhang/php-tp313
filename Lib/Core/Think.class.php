...
class Think {
    ...
    static private function buildApp() {
        ...
        if(isset($mode['core'])) {
			...
			if(class_exists('PHPUnit_Framework_TestCase')){
				array_push($list,CORE_PATH.'Core/UnitAction.class.php');
			}
        }
		...
		//本地有.env就不加载全局的.env
		if(!file_exists(ENTRY_PATH . '.env')){
			if(defined('SOLUTION_ENV_FILE') && file_exists(SOLUTION_ENV_FILE))
				C(include SOLUTION_ENV_FILE);
		}
		...
		if(is_dir(COMMON_PATH)){
			...
			while (($filename = $dir->read()) !== false){
				...
				if(substr($filename,-4) != '.php')
					continue;
				include COMMON_PATH . $filename;
				if(!APP_DEBUG)  
					$compile .= "require_once('" . COMMON_PATH . $filename . "');";
			}
			$dir->close();
		}
    ...
    public static function autoload($class) {
		...
		$clsinfo    =   explode('\\',$class); 
        /*if( substr($class,0,8)=='Jiaruan\\' || 
			substr($class,0,7)=='Nuantu\\' || 
			substr($class,0,4)=='Jms\\' || 
			substr($class,0,5)=='Rlfd\\'){ //如果是嘉软组件库
			load_jiaruan_class($class);
		}*/
		if(in_array($clsinfo[0],array('Jiaruan','Jms','Rlfd','Zndp','Dcjk'))){
			load_jiaruan_class($class);
            return;
		}
		if(defined('PHPCLS_NAMESPACE')){
            $namespaces = explode(',',PHPCLS_NAMESPACE);
            if(in_array($clsinfo[0],$namespaces) ){
			    load_jiaruan_class($class);
                return;
            }
		}
		if(substr($class,-8)=='Behavior') { // 加载行为
            /*
            if($class == 'CheckRouteBehavior'){
                echo "CORE_PATH" . CORE_PATH . "<BR>";
                echo "EXTEND_PATH" . EXTEND_PATH . "<BR>";
                echo "LIB_PATH" . LIB_PATH . "<BR>";
                echo "libPath" . $libPath . "<BR>";
                echo "file" .  $file ."<BR>";
                echo 'THINK_PATH' . THINK_PATH . "<BR>";
                require_cache(CORE_PATH.'Behavior/'.$file);
                //$a = new CheckRouteBehavior();
                //var_dump($a);
                //exit;
                return;
            }
