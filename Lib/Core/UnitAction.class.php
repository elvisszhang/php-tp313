 ...
 *
 * @author Mike.G
 * @url http://blog.csdn.net/u012225745/article/details/45293587 thinkphpunit教程
 * @url https://phpunit.de/manual/current/zh_cn/phpunit-book.html phpunit手册
 */
...
abstract class UnitAction extends Action
{
     ...
     * @var int
     */
    protected $_message_render = 4;  //默认输出HTML到屏幕
    ...
    public function setMessageRender($render=null)
    {
        ...
        }elseif ($render == self::MESSAGE_RENDER_HTML) {
            $this->_message_render = self::MESSAGE_RENDER_HTML;
        }else {
    ...
    private function __render($trace)
    {
        ...
        }elseif ($this->_message_render == self::MESSAGE_RENDER_HTML) {
            $this->__render_html($trace);
        }
    }
...
DATA;
    ...
    private function __render_html($trace)
    {
        $log = <<<DATA
        ...
        if (is_a($e, 'PHPUnit_Framework_ExpectationFailedException')) {
            ...
            foreach ($e->getTrace() as $trace) {
                if ($trace['class'] == $this_action_class_name) {
                    $action_trace = $trace;
                }elseif ('UnitAction' == $trace['class'] && '__call' == $trace['function']) {
                    $assert_trace = $trace;
                }
            }
