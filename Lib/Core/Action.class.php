...
abstract class Action {
   ...
   /**
    ...
    /**
     * 是否AJAX请求
     * @access public，原来protected，现在改成public方便外部调用
     * @return bool
     */
	...
	//查询预配置的空操作
	public function getEqualAction($method){
		foreach($this->equalMethods as $action => $data){
    ...
    public function __call($method,$args) {
        if( 0 === strcasecmp($method,ACTION_NAME.C('ACTION_SUFFIX'))) {
			//执行预定义等价操作
			if( $action = $this->getEqualAction($method) ){
