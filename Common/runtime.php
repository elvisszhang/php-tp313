...
function load_runtime_file() {
    ...
    $list = array(
        ...
        CORE_PATH.'Core/RollbackException.class.php',  // 异常处理类
        CORE_PATH.'Core/JrException.class.php',  // 嘉软开发平台自定义处理类
    ...
    if(!is_dir(LIB_PATH)) {
        // 创建项目目录结构
        build_app_dir();
    }elseif(!is_dir(CACHE_PATH)||!is_dir(TEMP_PATH)||!is_dir(DATA_PATH)){
        // 检查缓存目录
        check_runtime();
    }elseif(APP_DEBUG){
        // 调试模式切换删除编译缓存
...
function check_runtime() {
    ...
    if(!is_dir(CACHE_PATH)) mkdir(CACHE_PATH,0777,true);  // 模板缓存目录
    if(!is_dir(LOG_PATH))   mkdir(LOG_PATH,0777,true);    // 日志目录
    if(!is_dir(TEMP_PATH))  mkdir(TEMP_PATH,0777,true);   // 数据缓存目录
    if(!is_dir(DATA_PATH))  mkdir(DATA_PATH,0777,true);   // 数据文件目录
    return true;
}
...
function build_app_dir() {
    ...
    if(is_writeable(APP_PATH)) {
        ...
        header('Content-Type:text/html; charset=utf-8');
        exit('项目目录'.APP_PATH.'不可写，目录无法自动生成！(也有可能是LIB_PATH权限没设置好)<BR>请使用项目生成器或者手动生成项目目录~');
    }
}
