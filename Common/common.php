 ...
 * @return string
 */
function parse_name3($name, $type=0) {
    if ($type) {
...
/**
    给realpath支持phar协议
**/
function realpath_patch($filename){
...
function file_exists_case($filename) {
    if (is_file($filename)) {
        if (IS_WIN && C('APP_FILE_CASE')) {
            if (basename(realpath_patch($filename)) != basename($filename))
                return false;
        }
        return true;
    }
 ...
 * @param string $prefix 微服务前缀
 * @return Model
 */
function MM($name,$prefix = '') {
	//如果是继承模型,跳转模型
	...
	//跨模块加载模型
	return load_mservice_model($name,$prefix);
}

/**
 * MB 函数用于申明一个微服务的全局行为操作
 * @param string $name Model资源地址
 * @return Model
 */
function MB($name, &$data = NULL,$options = array() ) {
	if(strpos($name,'/')){
	...
	//全局的aop类
    $advisor = create_global_aop_advisor_class($name);
	if($advisor)
		$advisor->$method($data,$options);
}

/**
 * BB 函数用于申明一个微服务的本地行为操作
 * @param string $name Model资源地址
 * @return Model
 */
function BB($name, &$data = NULL,$options = array() ) {
	if(strpos($name,'/')){
        list($name,$method) = explode('/',$name);
    }else{
        $method = 'run';
	...
	//本微服务内的aop类
    $advisor = create_local_aop_advisor_class($name);
	if($advisor)
		$advisor->$method($data,$options); 
}

/**
 * PB 函数用于申明一个插件的切点操作（仅在 - 嘉软插件中心下可用）
 * @param string $name Model资源地址
 * @return Model
 */
function PB($name, &$data = NULL,$options = array() ) {
	//参数解析
	if(strpos($name,'/')){
        list($name,$method) = explode('/',$name);
    }else{
	...
	//获取对应接入点
	$advices_path = SOLUTION_DATA_PATH . 'advices.php';
	if(!file_exists($advices_path))
		return;
	$all_advices = require($advices_path);
	$advices = $all_advices[$name];
	
	//加载接入点类
	foreach($advices as $advice_name => $advice_info){
		if(!$advice_info['enabled'])
			continue;
		$class_name = camelCase($advice_name);
		$advice_path = str_replace('{solution.dir}',SOLUTION_PATH ,$advice_info['path']);
		if(!file_exists($advices_path)){
			//echo '接入点类不存在 ' . $advice_path;
			continue;
		}
		require_once($advice_path);
		$cls = aop_advice_instance( $advice_info['class'] );
...
function D($name='',$layer='') {
    ...
    $layer          =   $layer?$layer:C('DEFAULT_M_LAYER');
 	if(strpos($name,'_')) //增加支持linux命名法则的名称，自动转为驼峰命名
 ...
 * @return Model
 */
function M($name='', $tablePrefix='',$connection='DB_DSN') {
	//如果是继承模型,跳转模型
	if(file_exists(CONF_PATH . 'db_structure.php')){
		static $dbs = null;
		if($dbs === null)
    ...
    if (!isset($_model[$guid])){
		$_model[$guid] = create_dbaop_class($name,$connection);
		if(! $_model[$guid] ){
...
function L($name=null, $value=null) {
    ...
    if (is_string($name)) {
        $key = strtoupper($name);
        if (is_null($value))
            return isset($_lang[$key]) && $_lang[$key] ? $_lang[$key] : $name;
        $_lang[$key] = $value; // 语言定义
        return;
    }
    // 批量定义
    if (is_array($name))
...
//从配置中心获取配置值
function load_from_config_dsn($prefix,$name){
	//跳过系统的配置项
	if( in_array($prefix,array('tags','extends')) )
		return '';
	
	//没有配置DSN的话，返回空字符串
	$config_dsn = C('CONFIG_DSN');
	if(! $config_dsn)
		return '';
	$pos = strrpos($config_dsn,':');
	if($pos === false)
		throw_exception('CONFIG_DSN BAD FORMAT');
	
	$db_dsn = substr($config_dsn,0,$pos);
	$config_table = substr($config_dsn,$pos+1);
	
	//查询配置数据
	$cfg_model = M($config_table,null,$db_dsn);  //防止 DB_PREFIX 值的影响
	if( function_exists( 'load_config' ) ){
		$val = load_config($cfg_model,$prefix,$name);
		if($val === 'false')
			return false;
		elseif($val === 'true')
			return true;
	...
	$val = trim($cfg_model->where($where)->getField('CfgValue'));
	if($val === 'false')
		return false;
	elseif($val === 'true')
		return true;
...
function save_to_config_dsn($prefix,$name,$value){
	//没有配置DSN的话，返回空字符串
	$config_dsn = C('CONFIG_DSN');
	if(! $config_dsn)
		return '';
	...
	//防止 DB_PREFIX 值的影响
	$cfg_model = M($config_table,null,$db_dsn);
	$where = array(
		'CfgPrefix' => $prefix,
		'CfgName' => $name
	);
	if($cfg_model->where($where)->count()){
		$cfg_model->where($where)->setField('CfgValue',$value);
	}
	else{
		$where = array(
			'CfgPrefix' => $prefix,
		...
		$cfg_model->where($where)->add($data);
	}
}
/**
 * 获取和设置配置参数 支持批量定义
...
function C($name=null, $value=null) {
    ...
    if (is_string($name)) {
        if (!strpos($name, '.')) {
            ...
            if (is_null($value)){
				...
				if(!isset($_config[$name])){
					throw_exception('_INVALID_CONSTANT_' . ':' . $name);
				}
				return $_config[$name];
			}
            $_config[$name] = $value;
        ...
        if (is_null($value))
            return isset($_config[$name[0]][$name[1]]) ? $_config[$name[0]][$name[1]] : load_from_config_dsn($name[0],$name[1]);
...
function trace($value='[think]',$label='',$level='DEBUG',$record=false) {
    ...
    if('[think]' === $value){ // 获取trace信息
		...
		if(PHP_SAPI != 'cli') //非cli模式才保存，否则会造成内存泄漏
			$_trace[$level][]   = $info;
